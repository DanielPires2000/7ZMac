...
final class SevenZipService: ArchiveServiceProtocol {
    ...
    func listContents(archive: URL) async throws -> [ArchiveItem] {
        let output = try runProcess(arguments: ["l", "-slt", archive.path])
    ...
    func extract(archive: URL, to destination: URL) async throws {
        ...
        _ = try runProcess(arguments: ["x", archive.path, outputFlag, "-y"])
    }
    ...
    func compress(files: [URL], to archive: URL, options: CompressionOptions) async throws {
        ...
        _ = try runProcess(arguments: args)
    }
    
    // MARK: - Private
    
    /// Execute 7zz with the given arguments and return stdout as a String.
    private func runProcess(arguments: [String]) throws -> String {
        ...
        guard task.terminationStatus == 0 else {
            throw SevenZipError.processFailure(exitCode: task.terminationStatus)
        }
